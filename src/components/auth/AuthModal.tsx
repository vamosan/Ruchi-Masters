        ...
        <div className="p-3 bg-slate-100 border-b-2 border-slate-200 flex items-center justify-between text-xs shrink-0">
          <div className="flex items-center gap-2 truncate">
            ...
            {currentUser.role === 'admin' ? (
              ...
              <span className="px-2.5 py-0.5 rounded-lg bg-[#CCFF00] text-slate-950 font-black flex items-center gap-1 font-cabinet truncate">
                <span>{currentUser.teamLogo}</span>
        ...
        <div className="p-4 sm:p-6 space-y-4 overflow-y-auto flex-1">
          ...
          {activeTab === 'team' && (
            <div className="space-y-4">
                        ...
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                              <div className="font-bold truncate text-slate-900 font-cabinet">{team.name}</div>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <span className="px-2 py-0.5 rounded bg-slate-100 border border-slate-300 font-mono text-[10px] font-black text-slate-700">
                              {team.code}
                            </span>
                            {hasPin ? (
                              <span className="text-[10px] text-emerald-700 font-black bg-emerald-100 px-1.5 py-0.5 rounded flex items-center gap-0.5" title="PIN Protected">
                                <Lock className="w-2.5 h-2.5" /> Set
                              </span>
                            ) : (
                              <span className="text-[10px] text-amber-700 font-black bg-amber-100 px-1.5 py-0.5 rounded flex items-center gap-0.5" title="First Time Setup Required">
              ...
              {selectedTeam && (
                <div className="p-3 rounded-2xl bg-gradient-to-r from-slate-900 to-slate-950 text-white flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="text-2xl p-1.5 rounded-xl bg-slate-800 border border-slate-700">{selectedTeam.logo}</span>
                    <div>
                      <h4 className="font-black text-sm font-cabinet leading-tight text-[#CCFF00]">{selectedTeam.name}</h4>
                  ...
                  <span className="px-2.5 py-1 rounded-xl bg-[#00F0FF] text-slate-950 font-black text-xs font-mono">
                    {selectedTeam.code}
                  </span>
                </div>
              )}
              ...
              {authMode === 'setup_pin' && (
                <form onSubmit={handleSetNewPin} className="space-y-3 pt-1 border-t-2 border-slate-200">
                  ...
                  >
                    ...
                    <span>Set PIN & Enter {selectedTeam?.code || 'Squad'}</span>
                  </button>
                </form>
              )}
                ...
                <form onSubmit={handleTeamLogin} className="space-y-3 pt-1 border-t-2 border-slate-200">
                  ...
                  <div className="space-y-2 pt-1">
                    ...
                    >
                      ...
                      <span>Log In to Manage {selectedTeam?.code || 'Squad'}</span>
                    </button>
